...
class VoiceNotesApp {
  ...
  constructor() {
    ...
    this.copyPolishedNoteButton = document.getElementById(
        'copyPolishedNoteButton'
    ) as HTMLButtonElement;
    ...
    this.activeTabIndicator = tabNav.querySelector(".active-tab-indicator") as HTMLDivElement;
  ...
  private bindEventListeners(): void {
    ...
    this.copyPolishedNoteButton.addEventListener('click', () => this.copyPolishedNote());
  ...
  private handleFileUpload(event: Event): void {
    ...
    if (!file.type.startsWith('audio/') && !file.type.startsWith('video/')) {
        alert('Vui lòng chọn một tệp âm thanh hoặc video.');
        input.value = ''; // Reset input
        return;
    }

    this.processUploadedFile(file).catch(err => {
        console.error('Lỗi xử lý tệp đã tải lên:', err);
        this.recordingStatus.textContent = 'Lỗi xử lý tệp.';
        this.enableActionButtons();
    });
  ...
  private async processUploadedFile(file: File): Promise<void> {
    this.createNewNote(true); // Save current note if any, and reset UI for the new one.
    this.recordingStatus.textContent = 'Đang xử lý tệp...';
    this.recordButton.disabled = true;
    this.uploadButton.disabled = true;
    ...
    try {
        const base64Audio = await new Promise<string>((resolve, reject) => {
            ...
            reader.onloadend = () => {
                try {
                    ...
                    if (!base64AudioData) {
                        return reject(new Error('Không thể đọc dữ liệu tệp.'));
                    }
        ...
        console.error('Lỗi trong processUploadedFile:', error);
        this.recordingStatus.textContent = 'Lỗi xử lý tệp đã tải lên. Vui lòng thử lại.';
        this.enableActionButtons();
    }
  }
  ...
  private async getTranscription(
    ...
    try {
      ...
      if (transcriptionText) {
        ...
        this.polishedNote.appendChild(this.copyPolishedNoteButton);
  ...
  private async updatePolishedNoteUI(polishedText: string): Promise<void> {
    ...
    this.polishedNote.appendChild(this.copyPolishedNoteButton); // Ensure button is always at the end
    ...
    if (polishedText.trim() !== '') {
        ...
        const placeholder = this.polishedNote.getAttribute('placeholder') || '';
        this.polishedNote.innerHTML = placeholder;
        this.polishedNote.appendChild(this.copyPolishedNoteButton); // And here too
        this.polishedNote.classList.add('placeholder-active');
    }
  ...
  private setTheme(theme: 'light' | 'dark'): void {
      ...
      if (icon) {
          if (theme === 'dark') {
              ...
              this.themeToggleButton.title = 'Chuyển sang giao diện sáng';
          } else {
              icon.classList.remove('fa-sun');
              icon.classList.add('fa-moon');
              this.themeToggleButton.title = 'Chuyển sang giao diện tối';
          }
      }
  }
  ...
  private setActiveTab(activeButton: HTMLButtonElement, skipAnimation = false): void {
      ...
      this.noteContents.forEach((content) => {
        // We manage display with the active class, not just opacity.
        // So this logic from the old script is combined.
      ...
      if (skipAnimation) {
          this.activeTabIndicator.offsetHeight; // Trigger reflow
  ...
  private updateCopyButtonVisibility(): void {
    const isPolishedTabActive = this.polishedNote.classList.contains('active');
    const hasContent = !this.polishedNote.classList.contains('placeholder-active');

    if (isPolishedTabActive && hasContent) {
        this.copyPolishedNoteButton.classList.remove('hidden');
    } else {
        this.copyPolishedNoteButton.classList.add('hidden');
    }
  }
  ...
  private copyPolishedNote(): void {
    ...
    // Clone the node and remove the button to ensure clean text copy
    const noteClone = this.polishedNote.cloneNode(true) as HTMLElement;
    const buttonInClone = noteClone.querySelector('#copyPolishedNoteButton');
    if (buttonInClone) {
        buttonInClone.remove();
    }
    const textToCopy = noteClone.innerText;

    navigator.clipboard.writeText(textToCopy).then(() => {
      const icon = this.copyPolishedNoteButton.querySelector('i');
      if (icon) {
        icon.classList.remove('fa-copy');
        icon.classList.add('fa-check');
        this.copyPolishedNoteButton.title = 'Đã sao chép!';
        this.copyPolishedNoteButton.disabled = true;
      }
      setTimeout(() => {
        if (icon) {
          icon.classList.remove('fa-check');
          icon.classList.add('fa-copy');
          this.copyPolishedNoteButton.title = 'Sao chép nội dung';
          this.copyPolishedNoteButton.disabled = false;
        }
      }, 2000);
    }).catch(err => {
